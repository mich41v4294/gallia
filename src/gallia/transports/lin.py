...
from uds import Uds, LinTp # python-uds
...
class LINConfig(BaseModel):
    baudrate: int = 19200
    mode: str = "master"


class LINTransport(BaseTransport, scheme="lin-raw"):
    def __init__(self, target: TargetURI, config: LINConfig, connection: LinTp) -> None:
        super().__init__(target)
        self._connection = connection 
        self.config = config

    @classmethod
    async def connect(cls, target: str | TargetURI, timeout: float | None = None) -> LINTransport:
        t = target if isinstance(target, TargetURI) else TargetURI(target)
        cls.check_scheme(t)

        # Extract LIN-specific configuration from TargetURI
        #baudrate = t.qs_flat.get("baudrate", 19200)  # Default baudrate
        #interface = t.qs_flat.get("interface", "peak") 

        connection = LinTp(baudrate=self.config.baudrate, interface=interface)
        connection.connect()  

        return cls(t, t.qs_flat, connection)

    async def read(self, timeout: float | None = None, tags: list[str] | None = None) -> bytes:
        # LIN read logic using python-uds
        response = await asyncio.wait_for(self._connection.recv(), timeout)  
        return response

    async def write(self, data: bytes, timeout: float | None = None, tags: list[str] | None = None) -> int:
        # LIN write logic using python-uds
        await asyncio.wait_for(self._connection.send(data), timeout)
        return len(data)
